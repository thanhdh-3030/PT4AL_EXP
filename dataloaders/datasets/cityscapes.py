# get from https://github.com/NoamRosenberg/autodeeplab
import os
import numpy as np
...
class CityscapesSegmentation(data.Dataset):
    ...
    def __init__(self, args, root='', split="train", indices_for_split=None):
        ...
        if split.startswith('re'):
            self.images_base = os.path.join(self.root, 'images', self.split[2:])
        ...
        # print(type(self.files[split]))
    def __len__(self):
        return len(self.files[self.split])

    def __getitem__(self, index):
        # print(self.files[self.split][index])
...
class ActiveCityscapesSegmentation(data.Dataset):
    ...
    # def __init__(self, args, root=Path.db_root_dir('cityscapes'), split="train", indices_for_split=None):
    def __init__(self, args, root='', files=[],split="train", indices_for_split=None):
        self.root = root
        self.split = split
        self.args = args
        self.files = {}
